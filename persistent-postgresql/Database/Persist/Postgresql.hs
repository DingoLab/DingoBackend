...
instance PGTF.ToField P where
    ...
    toField (P (PersistList l))        = PGTF.toField $ PG.PGArray l
    toField (P (PersistMap m))         = PGTF.toField $ mapToJSON m
    toField (P (PersistDbSpecific s))  = PGTF.toField (Unknown s)
    toField (P (PersistObjectId _))    =
        error "Refusing to serialize a PersistObjectId to a PostgreSQL value"

-- PersistList to String
-- DO NOT USE WITH PersistByteString and PersistNull
listToPGAT :: [PersistValue] -> [String]
listToPGAT [] = []
listToPGAT (PersistText x:xs) = unpack x:listToPGAT xs
listToPGAT (PersistByteString _:xs) = error
listToPGAT (Persist x:xs) =
listToPGAT (Persist x:xs) =
listToPGAT (Persist x:xs) =
listToPGAT (Persist x:xs) =
listToPGAT (Persist x:xs) =
